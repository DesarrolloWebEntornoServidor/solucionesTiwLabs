...
public class LoggerFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		sb.append(SEPARADOR);
		sb.append(
				((HttpServletRequest) request).getProtocol() + SEPARADOR + ((HttpServletRequest) request).getMethod());
		sb.append(SEPARADOR);
		String url = request.getScheme() + "://" + request.getServerName() + request.getLocalPort()
				+ ((HttpServletRequest) request).getContextPath() + ((HttpServletRequest) request).getServletPath();
		sb.append(url);
